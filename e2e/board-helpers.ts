...
export function screenToNorm(sx: number, sy: number, host: Box, fit: Fit): Pt {
  ...
  return { x: (cx - offX) / s / RECT.w + (0 - RECT.x) / RECT.w, y: (cy - offY) / s / RECT.h - RECT.y / RECT.h };
}
...
export async function seedBoard(page: Page, opts: { players?: boolean } = {}): Promise<void> {
  ...
  await page.addInitScript((withPl) => {
    ...
    localStorage.setItem('entrenolab:players', JSON.stringify(withPl ? [
...
export async function dragDraw(page: Page, from: [number, number], to: [number, number]): Promise<void> {

...
async function seed(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    localStorage.setItem('entrenolab:exercises', JSON.stringify([
      {
        id: 'e1', teamId: 't1', folderId: 'f2', title: 'Rondo 4v2', description: 'Conservación del balón con superioridad.',
        explanation: '4 poseedores contra 2 defensas.', category: 'Técnica', objectives: [], materials: [],
        durationMinutes: 12, minPlayers: 6, maxPlayers: 8, loadMode: 'fixed', seriesCount: null, repetitionsCount: null,
        workSeconds: null, restSeconds: null, isTemplate: false, canvas: { version: 2, schemaVersion: 3, field: 'half', frames: [], orientation: 'horizontal', grass: 'stripes', lineColor: '#ffffff', backgroundColor: '#31834a' }, thumbnail: null, savedAt: now,
      },
    ]));
...
test.describe('Capturas baseline', () => {
  ...
  test('captura 6 campos (full/half/blank) × orientación (horizontal/vertical) para la revisión visual', async ({ page }) => {
    await page.setViewportSize({ width: 1366, height: 900 });
    await seed(page);
    await page.goto('/board');
  ...
  test('captura limpia: curva seleccionada con manijas separadas', async ({ page }) => {
    ...
    // Fase 3: clic derecho selecciona el elemento y abre el inspector (Propiedades).
    // Se pulsa sobre el origen de la curva, un punto que está SIEMPRE sobre el trazo.
    const [selX, selY] = normToScreen(0.52, 0.51, box);
  ...
  test('captura limpia: material (pica coloreable) seleccionado', async ({ page }) => {
    await page.setViewportSize({ width: 1366, height: 900 });
    await seed(page);
    await page.goto('/board');
    const box = (await page.locator('.board-host').boundingBox())!;
    ...
    await page.locator('.rail-btn[title="Pica coloreable"]').click();

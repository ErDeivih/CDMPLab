...
async function setPaletteColor(
  page: Page,
  hex: string,
  scope: '.tools-caption' | '.studio-panel .inspector',
): Promise<void> {
  const index = [
    '#1a73e8',
    '#c0392b',
...
test('color de línea: rojo → azul, undo/redo, duplicar, persiste en guardar/reabrir, PNG y respaldo', async ({
  page,
}) => {
  ...
  const a = normToScreen(0.25, 0.4, host);
  const b = normToScreen(0.75, 0.4, host);
  await page.mouse.move(a[0], a[1]);
  await page.mouse.down();
  await page.mouse.move(b[0], b[1], { steps: 5 });
  await expect(
    page.locator('.board-canvas svg line[stroke="#c0392b"]'),
    'previsualización roja',
  ).not.toHaveCount(0);
  await page.mouse.up();
  ...
  const strokes = await page
    .locator('.board-canvas svg [data-el-type="line"]')

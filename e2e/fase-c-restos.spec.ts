...
function realPlayer(): unknown {
  return { id: 'pl-1', teamId: 't1', name: 'David', number: 10, position: 'MC', color: '#1f7a4d', active: true, createdAt: new Date().toISOString() };
}
...
async function seedLegacyAndOpen(page: Page): Promise<void> {
  const legacy = {
    id: 'legacy1', teamId: 't1', folderId: null, title: 'Antiguo', description: '', explanation: '', category: 'Técnica',
    objectives: [], materials: [], durationMinutes: 15, minPlayers: null, maxPlayers: null, loadMode: 'fixed',
    ...
    canvas: {
      ...
      frames: [{ duration: 1000, elements: [
        { id: 'gkRival', t: 'player', x: 0.5, y: 0.5, n: 1, c: '#c0392b', side: 'rival', type: 'goalkeeper' },
      ] }],
      orientation: 'horizontal', grass: 'stripes', lineColor: '#ffffff', backgroundColor: '#31834a',
    },
    thumbnail: null, savedAt: new Date().toISOString(),
  };
...
test.describe('FASE C — restos de jugadores antiguos', () => {
  test('las cinco fichas por color colocan jugadores genéricos SIN nombre/dorsal/side/type', async ({ page }) => {
    ...
    for (const [, label, drop] of [
      ...
      const id = 91;
      await page.evaluate(({ sel, sx, sy, dropP, id }) => {
        const el = document.querySelector(sel) as HTMLElement | null;
        if (!el) return;
        el.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true, pointerId: id, pointerType: 'touch', isPrimary: true, button: 0, buttons: 1, clientX: sx, clientY: sy }));
        el.dispatchEvent(new PointerEvent('pointermove', { bubbles: true, cancelable: true, pointerId: id, pointerType: 'touch', isPrimary: true, button: 0, buttons: 1, clientX: dropP.x, clientY: dropP.y }));
        el.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, cancelable: true, pointerId: id, pointerType: 'touch', isPrimary: true, button: 0, buttons: 0, clientX: dropP.x, clientY: dropP.y }));
      }, { sel, sx, sy, dropP, id });
    }
    await expect.poll(() => fieldCount(page), { timeout: 5000 }).toBe(5);
    // Modelo: los genéricos NO llevan nombre, dorsal (n), side ni type.
    const players = await page.locator('.entrenolab-board circle[r="2.5"]').evaluateAll((els) =>
      els.map((e) => ({ fill: (e as SVGCircleElement).getAttribute('fill'), num: (e as SVGElement).querySelector('text')?.textContent ?? '' }))
    );
    expect(players.length).toBe(5);
    expect(players.every((p) => p.num === ''), 'ningún genérico muestra dorsal/nombre').toBe(true);
  });
  ...
  test('cada formación funciona sin plantilla y se crea en los cinco colores', async ({ page }) => {
    ...
    const colors: Array<[string, string]> = [
      ['#1a73e8', 'Azul'], ['#c0392b', 'Rojo'], ['#e6b800', 'Amarillo'], ['#1f7a4d', 'Verde'], ['#b884ff', 'Morado'],
    ];
    ...
    for (const [hex] of colors) {
      expect(await page.locator(`.entrenolab-board circle[r="2.5"][fill="${hex}"]`).count()).toBe(11);
    }
  });
  ...
  test('el inspector de jugador NO contiene la opción Portero ni el campo Lado', async ({ page }) => {
    await page.setViewportSize({ width: 1366, height: 900 });
  ...
  test('no existe ningún botón visible Jugador propio/rival/Portero', async ({ page }) => {
    ...
    const bad = await page.locator('[title="Jugador propio"], [title="Jugador rival"], [title="Portero"], [aria-label="Jugador propio"], [aria-label="Jugador rival"], [aria-label="Portero"]').count();
    expect(bad).toBe(0);
  });

  test('un jugador real de plantilla conserva nombre, dorsal, playerId y color', async ({ page }) => {
    await page.setViewportSize({ width: 844, height: 390 });
    await seed(page, [realPlayer()]);
    await openBoard(page);
    await page.locator('.tools-cat', { hasText: 'Jugadores' }).click();
    ...
    // El jugador real conserva su color (#1f7a4d) y su dorsal "10" se renderiza.
    expect(await page.locator('.entrenolab-board circle[r="2.5"][fill="#1f7a4d"]').count()).toBe(1);
  ...
  test('un documento ANTIGUO (side rival + type goalkeeper) abre y renderiza sin error', async ({ page }) => {
    await page.setViewportSize({ width: 1366, height: 900 });

...
test.describe('Fase multiusuario — capturas del estado actual', () => {
  ...
  // -------------------------------------------------------------------------
  test('auth: login, register, pending-approval, forgot-password, onboarding-team (desktop)', async ({
    page,
  }) => {
  ...
  test('members, admin, invitations (desktop) — estados vacíos de desarrollo', async ({ page }) => {
    ...
    // Sin propietario autenticado no se puede mostrar un contador de plazas verificable.
    // Antes la captura esperaba «0 de 6» aunque no existía una cuenta propietaria local.
    await page.goto('/settings/team/members');
    await expect(page.locator('.auth-title')).toHaveText('Miembros del equipo');
    await expect(page.locator('.auth-row')).toHaveCount(0);
    await expect(page.locator('.auth-note').first()).toContainText('Solo el propietario');
    await page.waitForTimeout(250);
  ...
  test('board móvil 390×844: cerrada + Material (bottom-sheet) + Propiedades', async ({ page }) => {
    ...
    // Material. CONTRATO ACTUALIZADO (fase shell+móvil): en móvil VERTICAL los cuatro paneles son
    // una HOJA INFERIOR (55-60 % de alto, ancho completo, anclada abajo) precisamente porque un
    // panel lateral deja el campo demasiado estrecho. Este test exigía un panel lateral de ≥80 % de
    // alto: ese era el contrato anterior y dejó de ser válido por requisito explícito del encargo.
    // El panel lateral se sigue comprobando en móvil HORIZONTAL (spec `fase-shell-movil`).
    await abrirHerramientas(page);
    ...
    expect(mat.width, 'la hoja ocupa todo el ancho').toBeGreaterThanOrEqual(MOBILE.width - 2);
    expect(mat.height, 'alto máximo ~58 % de la pantalla').toBeLessThanOrEqual(
      MOBILE.height * 0.6 + 2,
    );
    expect(mat.y, 'se ancla en la parte inferior').toBeGreaterThan(f0.y);
  ...
  test('F7: plantilla base compuesta (sobre medio campo F11), desktop 1366×900', async ({
    page,
  }) => {
    await page.setViewportSize(DESKTOP);
    await seed(page);

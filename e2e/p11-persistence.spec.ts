...
import { longPress, fillBoardTitle, abrirAjustes, toggleFillScreen } from './gesture-helpers';
...
async function resizeSelected(
  page: Page,
  el: CanvasElement,
  kind: 'box' | 'point' | 'seg' | 'curve' | 'freehand',
): Promise<void> {
  ...
  await page.evaluate(() => {
    ...
    (window as unknown as { __resizeOriginals: Array<[HTMLElement, string]> }).__resizeOriginals =
      originals;
  });
  ...
  if (kind === 'box') {
    ...
    await dragHandle(
      page,
      [x + w, y + h],
      [Math.min(0.9, x + w + 0.1), Math.min(0.9, y + h + 0.12)],
    );
  } else if (kind === 'point') {
    await dragHandle(
      page,
      [(el.x ?? 0) + 0.055, (el.y ?? 0) + 0.055],
      [(el.x ?? 0) + 0.14, (el.y ?? 0) + 0.16],
    );
  } else if (kind === 'seg') {
    await dragHandle(
      page,
      [el.x2 ?? 0.6, el.y2 ?? 0.6],
      [Math.min(0.9, (el.x2 ?? 0.6) + 0.16), Math.min(0.9, (el.y2 ?? 0.6) + 0.1)],
    );
  } else if (kind === 'curve') {
    await dragHandle(
      page,
      [el.x2 ?? 0.6, el.y2 ?? 0.6],
      [Math.min(0.9, (el.x2 ?? 0.6) + 0.14), Math.min(0.9, (el.y2 ?? 0.6) + 0.08)],
    );
    await dragHandle(
      page,
      [el.c1x ?? 0.4, el.c1y ?? 0.3],
      [Math.min(0.9, (el.c1x ?? 0.4) + 0.08), Math.max(0.1, (el.c1y ?? 0.3) - 0.06)],
    );
...
test.describe('Fase 11 — round-trip de modelo y persistencia por familia', () => {
  for (const fam of FAMILIES) {
    test(`familia «${fam.name}»: crear→mover→redimensionar→duplicar→guardar→salir→reabrir preserva el modelo`, async ({
      ...
      expect(
        Math.hypot(movedCentroid[0] - beforeCentroid[0], movedCentroid[1] - beforeCentroid[1]),
  ...
  test('round-trip: rotación exacta ±90° y propiedades (color/opacidad/capa) congeladas al guardar y reabrir', async ({
    page,
  }) => {
    await seed(page);
    await createFamily(
      page,
      FAMILIES.find((f) => f.name === 'rect')!,
    );
    await save(page);
    await reopen(page);
    await selectAt(page, (await savedElements(page))[0]);
    const colSwatch = page
  ...
  test('un jugador de plantilla (playerId) preserva su enlace real y NO se duplica (invariante)', async ({
    page,
  }) => {
    await seed(page);
    await abrirHerramientas(page);
  ...
  test('aviso de cambios sin guardar al salir con la pizarra sucia (y cancelar permanece)', async ({
    page,
  }) => {
    await seed(page);
    await createFamily(
      page,
      FAMILIES.find((f) => f.name === 'cone')!,
    );
  ...
  test('exportación/importación de respaldo conserva el ejercicio con su pizarra', async ({
    page,
  }) => {
    await seed(page);
    await createFamily(
    ...
    await expect
      .poll(
        () =>
          page.evaluate(
            () =>
              (JSON.parse(localStorage.getItem('entrenolab:exercises') ?? '[]') as unknown[])
                .length,
          ),
        { timeout: 5000 },
      )
    ...
    const restored = await page.evaluate(() =>
      JSON.parse(localStorage.getItem('entrenolab:exercises')!),
    );
  ...
  test('el PNG exportado refleja el campo guardado (firma, dimensiones del campo horizontal y sin controles del editor)', async ({
    page,
  }) => {
    await seed(page);
    await createFamily(

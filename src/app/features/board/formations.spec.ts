...
describe('formations (módulo puro — formaciones rápidas independientes de la plantilla)', () => {
  ...
  it('todos los jugadores usan el color recibido y no exponen playerId ni label', () => {
    const specs = buildFormationPlayers('#c0392b', '4-3-3')!;
    for (const s of specs) {
      expect(s.c).toBe('#c0392b');
      expect(s.side).toBe('own'); // sin espejo => geometría original
      // El spec puro no expone playerId/label: los jugadores son genéricos.
  ...
  it('con reflectRival=true refleja en X y marca side rival (diferenciación por color, no por rol)', () => {
    const specs = buildFormationPlayers('#e6b800', '4-4-2', true)!;
    expect(specs).toHaveLength(11);
    expect(specs.every((s) => s.side === 'rival')).toBe(true);
